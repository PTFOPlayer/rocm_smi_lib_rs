...
use crate::{error::{IntoRocmErr, RocmErr}, MapWithString, RocmSmi};
...
impl RocmSmi {
    ...
    pub fn get_device_identifiers(&mut self, dv_ind: u32) -> Result<Identifiers, RocmErr> {
        ...
        let revision = unsafe {
            rsmi_dev_revision_get(dv_ind, &mut revision_data as *mut u16)
                .into_rocm_err()
                .map(|_| revision_data)
        };
        ...
        let name = unsafe {
            ...
            Into::<RocmErr>::into(rsmi_dev_name_get(dv_ind, buff, NAME_SIZE)).map_with_buff(buff)
        };

        let brand = unsafe {
            let buff = libc::malloc(NAME_SIZE).cast();
            Into::<RocmErr>::into(rsmi_dev_brand_get(dv_ind, buff, NAME_SIZE as u32)).map_with_buff(buff)
        };

        let vendor_name = unsafe {
            let buff = libc::malloc(NAME_SIZE).cast();
            Into::<RocmErr>::into(rsmi_dev_vendor_name_get(dv_ind, buff, NAME_SIZE)).map_with_buff(buff)
        };
        let vram_vendor_name = unsafe {
            let buff = libc::malloc(NAME_SIZE).cast();
            Into::<RocmErr>::into(rsmi_dev_vram_vendor_get(dv_ind, buff, NAME_SIZE as u32)).map_with_buff(buff)
        };
        let serial_number = unsafe {
            let buff = libc::malloc(NAME_SIZE).cast();
            Into::<RocmErr>::into(rsmi_dev_serial_number_get(dv_ind, buff, NAME_SIZE as u32)).map_with_buff(buff)
        };

        let subsystem_name = unsafe {
            let buff = libc::malloc(NAME_SIZE).cast();
            Into::<RocmErr>::into(rsmi_dev_subsystem_name_get(dv_ind, buff, NAME_SIZE)).map_with_buff(buff)
        };

        let mut xgmi_physical_id_data = 0u16;
        let xgmi_physical_id = unsafe {
            rsmi_dev_xgmi_physical_id_get(dv_ind, &mut xgmi_physical_id_data as *mut u16)
                .into_rocm_err()
